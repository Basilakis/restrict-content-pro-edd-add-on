...
class RCP_EDD_HANNANStd {
	...
	public function __construct() {
		...
		if ( is_admin() ) {	
			add_action( 'admin_init', 						array( $this, 'rcp_edd_admin_init') );
	...
	public function rcp_edd_alter_table(){
		...
		if ( $wpdb->get_var( "show tables like '$rcp_db_name'" ) == $rcp_db_name && ! get_option( 'rcp_edd_is_installed') ) {
			...
			add_option( 'rcp_edd_is_installed', '1' );
	
		}	
	
	}
	
	public function rcp_edd_admin_init() {
		if ( ! get_option( 'rcp_edd_is_installed') ) {
			self::rcp_edd_alter_table();
		}
	}

...
class EDD_RCP_HANNANStd {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			...
			add_filter( 'edd_save_download', 			 	array( $this, 'edd_rcp_save_download' ) );
	...
	public function edd_rcp_get_download_price( $price, $download_id ) {
		...
		if ( is_admin() || ! is_user_logged_in() || self::edd_rcp_ignore_download($download_id))
			return $price;
	...
	public function edd_rcp_purchase_link_args( $args ) {
		...
		if ( is_admin() || ! is_user_logged_in() || self::edd_rcp_dont_show_main_price( $args['download_id'] ) || self::edd_rcp_ignore_download( $args['download_id'] ) )
			return $args;
	...
	public function edd_rcp_download_price_after_html( $formatted_price, $download_id, $price, $price_id ) {
		...
		if ( is_admin() || ! is_user_logged_in() || self::edd_rcp_dont_show_main_price( $download_id ) || self::edd_rcp_ignore_download( $download_id ) )
	...
	public function edd_rcp_cart_item_price_label( $label, $item_id, $options ) {
		...
		if ( is_admin() || ! is_user_logged_in() || self::edd_rcp_dont_show_main_price( $item_id ) || self::edd_rcp_ignore_download( $item_id ) )
			return $label;
							...
							<label for="<?php echo esc_attr( 'edd_price_option_' . $download_id . '_' . $key ); ?>">
								...
								<input type="<?php echo $type; ?>" <?php checked( apply_filters( 'edd_price_option_checked', $checked_key, $download_id, $key ), $key ); ?>
									...
									<span class='edd_price_option_wrap'>
										...
										if ( ! is_admin() && is_user_logged_in() && isset( $price['main_amount'] ) && !self::edd_rcp_ignore_download( $download_id ) && !self::edd_rcp_dont_show_main_price( $download_id )  ) {
